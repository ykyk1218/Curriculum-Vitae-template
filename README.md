...
| 役割                 | メンバー構成      | 内容                                                         |
| -------------------- | ----------------- | ------------------------------------------------------------ |
| プロジェクトリーダー | ・PM:1名<br />・プログラマー（バックエンド）:3名<br />フロントエンド:2名<br />デザイナー:１名| 外部サービスのAPIを使って商品データを連携する必要があったのですが、そもそもの仕組みをあまりわかっていなかったりするためかなり不確実性が高いプロジェクトでした。<br />おおよその期限は決められていたため、まず最初にリリースまでのマイルストーン（スプリントゴール）を決めました。スプリントゴールに従って進めることで、進捗が遅れているかどうかというのがメンバー感で共通認識を持つことができたと思います。<br />プロジェクト前半はAPIの仕様調査や実際思った通りに動かなかったりで苦戦をしていたのですが、短期目標がはっきりしたことで当初のスケジュール通りに終わらせることができました。
...
| 役割                 | メンバー構成      | 内容                                                         |
| -------------------- | ----------------- | ------------------------------------------------------------ |
| プロジェクトリーダー | ・PM<br / >1名<br />・プログラマー3名| 既存の処理フローに追加で処理を加える、というプロジェクトでした。<br />しかし既存の処理をきちんと全部把握している人はいなかったため、まずコードを読むことで既存の処理を正確に把握するところから始まりました。<br />また入金周りの処理ということもあって重要なところであるのですが、非常に複雑な仕様なため理解を深めるためと、他の人が理解をしやすいようにするためにドキュメントをきちんと残すようにしました。ドキュメントは技術者向けのものとお問い合わせがあったときに説明ができるようカスタマーサポート向けにドキュメントを作成し、勉強会などを開き内容を共有する時間を設けました。

- Elasticsearch のインデックス変更

| 役割                 | メンバー構成      | 内容                                                         |
| -------------------- | ----------------- | ------------------------------------------------------------ |
| プロジェクトリーダー | ・プログラマー3名| あるプロジェクトで商品データに対してフィールドを追加する必要があり、その中で追加したフィールドで検索ができるようにする必要がありました。検索にはElasticsearchを使っていたのですが、今の仕組みだと1つの情報を更新するたびに複数のドキュメントを更新する必要が出てきてしまい、動きが若干複雑になってしまったり、更新処理が効率悪くなってしまったりと技術負債になってしまうため、構造を変更することを決めました。Elasticsearchについて知見があるメンバーはいなかったので、手探りで調べながら変更後にパフォーマンスに問題がないことや、リリースの手順書を作ってデータ移行が問題なく進むようにしました。
